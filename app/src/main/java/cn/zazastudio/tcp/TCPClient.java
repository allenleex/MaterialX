...
public class TCPClient {
    ...
    public void connect(String ip, int port) {
        try {
            ...
            handler.sendEmptyMessage(TCPHandler.CONNECT_SUCCESS);
            flag = true;
            readServer();
        } catch (IOException e) {
            e.printStackTrace();
            flag = false;
            handler.sendEmptyMessage(TCPHandler.CONNECT_FAILED);
        }
    }
    ...
    private void readServer() {
        while (flag) {
            try {
                ...
                message = new Message();
                message.what = TCPHandler.READ_DATA;
    ...
    public void sendMessage(String messageText) {
        try {
            if (!flag || !TCPHandler.CONNECT_STATUS) return;
            byte[] data = messageText.getBytes();
            outputStream.write(data);
            outputStream.flush();
            message = new Message();
            message.what = TCPHandler.WRITE_DATA;
